# Brief 30C — figures for docs/analytic, lane C: `docs/analytic/parts/part2a.tex` (Part II §7–10)
...
- You MAY edit: `docs/analytic/parts/part2a.tex` and create files `docs/analytic/figs/fig-<slug>.tex` for the slugs assigned to you below (and extra ones you add, prefixed with your lane letter if the slug could collide: `fig-C-<slug>.tex`).
...
- Compile ONLY into your own build directory: `cd docs/analytic && mkdir -p build/laneC && pdflatex -interaction=nonstopmode -halt-on-error -output-directory=build/laneC analytic-underpinnings.tex` (run twice). Never compile into `docs/analytic/` itself (shared aux files).
- Do not run git. Do not run Julia.

## Read order
1. `docs/analytic/FIGURES.md` — the visual system (§1, binding rules 1–12), the coverage gate (§2), and YOUR figure plan: **F36–F58** under "Lane C" (§3). ★ figures are mandatory; others may be replaced by a better figure of the same object if you explain why.
...
3. Your part file `docs/analytic/parts/part2a.tex` in full. Every figure depicts an object the text actually defines, with the text's notation and numbers.
...
## Procedure
1. Compile the current document into `build/laneC` and run `python3 tools/figcoverage.py build/laneC/analytic-underpinnings.aux --from 17 --to 31` to see your pages.
...
4. Self-review each figure against the craft bar; look at the PDF pages (`pdftoppm -r 60 -f P -l P build/laneC/analytic-underpinnings.pdf out` then inspect with any image-viewing means available to you; if none, inspect the TikZ geometry by reasoning) and fix overlaps and misalignments.

## Report: `briefs/30C-figs-part2a.last.md` (write it yourself at the end; ≤ 60 lines)
