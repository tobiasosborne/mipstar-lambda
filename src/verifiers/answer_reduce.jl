...
# The honest prover reads its line through the SAME `axis_line`/`diagonal_line`
# the decider uses (fig:ld-decider items 2-3), so an unprojected DLine
# direction is projected by pi_{chi(s)-1} on both sides by construction.
_question_line(question::PCPALineQuestion{F,N}) where {F,N} =
    axis_line(pcp_ld_question(question), N)
_question_line(question::PCPDLineQuestion{F,N}) where {F,N} =
    diagonal_line(pcp_ld_question(question), N)
