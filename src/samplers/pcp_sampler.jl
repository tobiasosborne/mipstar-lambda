...
struct PCPRegisterLayout
    ...
    registers::Dict{Tuple{Int,Symbol},Vector{Int}}
...
function _pcp_register_layout(params::PCPParams)
    ...
    cursor = 0
    for i in 1:5
        registers[(i, :pt)] = collect(cursor+1:cursor+params.m)
        cursor += params.m
        registers[(i, :coord)] = [cursor + 1]
        cursor += 1
        registers[(i, :dir)] = collect(cursor+1:cursor+params.m)
        cursor += params.m
    end
    auxiliary_width = 5 + params.s
    auxiliary_point = collect(cursor+1:cursor+auxiliary_width)
    cursor += auxiliary_width
    auxiliary_coordinate = cursor + 1
    cursor += 1
    auxiliary_direction = collect(cursor+1:cursor+auxiliary_width)
    cursor += auxiliary_width
    registers[(6, :pt)] = vcat([registers[(i, :pt)] for i in 1:5]...,
                               auxiliary_point)
    registers[(6, :coord)] = vcat([registers[(i, :coord)] for i in 1:5]...,
                                  [auxiliary_coordinate])
    registers[(6, :dir)] = vcat([registers[(i, :dir)] for i in 1:5]...,
                                auxiliary_direction)
...
function _build_pcp_sampler(::Type{F}, params::PCPParams) where {F<:GF2k}
    ...
    types = [PCPType(kind, i) for kind in _PCP_KINDS for i in 1:6]
    ...
    graph = [(left, right) for left in types for right in types]
...
precompile(apply, (CLStep{GF2048}, NTuple{40,GF2048}))
precompile(apply, (CLStep{GF2048}, NTuple{2,GF2048}))
