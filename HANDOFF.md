...
| TB1 | r0 landed; C4a TESTED; repair r1 (brief 21) INTERRUPTED by the user's stop — partial state committed: suite 220/221 assertions green (only the cold-precompile timing gate tripped), 6 new TB1 mutant files added, mutation runner NOT re-run | tb1-r1 FAIL(O1,O2,O3,O5) | resume brief 21 from its response table (`briefs/21-tb1-repair.last.md` may be absent: reconstruct from `git diff 4a3474c HEAD -- src test`); run mutations; then critic r2 for TB1 and TB2 |
