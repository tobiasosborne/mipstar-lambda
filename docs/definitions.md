...
| `lambda` (fixed-point context) | The resource-bound parameter of `def:lambda`, with term sort `P{Nat}` inside `Psi_{M,lambda}`; it is supplied to `Compress` and to `FuelBound(n, lambda)`; the code name is `lambda` (DESIGN §1.1; brief 24). |
