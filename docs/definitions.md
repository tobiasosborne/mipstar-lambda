...
| `IntroAnswerEncoding` | The repaired executable convention serializes every non-Pauli vector field as a full `Q_EPR`-bit vector, zero outside the first `s(N)` coordinates. It does not use an `s(N)`-bit alternative. | `gt-08-introspection.tex:L401-L416`, `L501-L538`, `L588-L591`; DESIGN §11.4. |
