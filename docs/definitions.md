...
| `n` | External verifier index. In `Psi_{M,lambda}`, its bound occurrence has term sort `P{Nat}`. In `prop:zero-basis` only, the paper also uses `n` for polynomial arity; code uses `arity` there. | `gt-10-answer-reduction.tex:L1378-L1387` (`sec:ar-pcp`); `L1281-L1293` (`prop:zero-basis`). |
...
| `Level` | Finite serializable compression-level data (a CL level, `level(V)`). The fixed-point parameter is NOT a level: it is `lambda : P{Nat}`, the resource bound of `def:lambda` (`gt-05-games-normalform.tex:L641-L653`), passed unchanged to `Compress` (DESIGN §1.1, renamed 2026-09-05 per `verdicts/analytic-doc-r1.md` M1). | `gt-04-cl.tex:L36-L62` (`def:cl-func`, ℓ-level CL functions); `gt-05-games-normalform.tex:L624-L634` (`def:normal-ver`: the number of levels of `V` is the number of levels of its sampler). |
| `Compressor` | Program-result sort for a transformation that accepts the quoted sampler/decider pair and `lambda : P{Nat}`, the resource bound of `def:lambda`, and returns a decider description — a named, deliberate narrowing of `thm:compression`'s pair-valued output, licensed by `fig:halt_f` step 3 (`S^halt = ComputeSampler(lambda)`) with `lem:dhalt-values` and `lem:compress-independent-samplers` (`V^halt_n` and `V^compr_n` use the same sampler `S^compr`), not by this row (`verdicts/tb4-r1.md` §5 MP-2 (i)). | Project sort for the compression interface at `gt-12-compression.tex:L26-L39`; `gt-12-compression.tex:L440-L453` (`fig:halt_f`), `L502-L519` (`lem:dhalt-values`), `L108-L118` (`lem:compress-independent-samplers`). |
...
| `Fuel` | Evaluator-fuel syntax `FuelLiteral(Nat)` or `FuelBound(P{Nat},P{Nat})`, the second operand the resource bound `lambda` of `def:lambda`; evaluation interprets it to a finite bound `n^lambda` (an overflowing power is a budget not below the remaining ambient fuel: `OutOfFuel`, never `TypeError`) or returns `TypeError` (Julia: `SortError`) on ill-sorted operands. Thus `FuelBound(n,lambda)` in `Psi_{M,lambda}` is well-sorted. | Project surface sort for bounded evaluation at `gt-10-answer-reduction.tex:L200-L205`. |
...
| `Psi_{M,lambda}` (formerly `Psi_{M,L}`) | Quoted functional whose fixed point accepts if machine `M` halts within `n`, otherwise invokes the compressed verifier built from its own quoted description and `S_lambda`. | Mandate equation in `handoff.md:L21-L35`; paper compression interface at `gt-12-compression.tex:L26-L39`. |
| `D_{M,lambda}` (formerly `D_{M,L}`) | Closed fixed-point decider `Fix(Psi_{M,lambda})`. | Mandate equation in `handoff.md:L31-L39`; corresponding compression machinery `gt-12-compression.tex:L26-L53`. |
| `S_lambda` (formerly `S_L`) | Canonical executable compressed-sampler description computed from `lambda` alone and paired with the self-referential decider in the halting construction. Its static dependency set excludes the input verifier. | Mandate `handoff.md:L23-L35`; `gt-12-compression.tex:L108-L147` (`lem:compress-independent-samplers`). |
| `M` (machine context) | The Turing machine whose halting is tested, with term sort `P{MachineDesc}` inside `Psi_{M,lambda}`. Code calls it `machine` to avoid collision with assignment length `M=2^m`. | Mandate `handoff.md:L21-L35`; bounded-halting description `gt-10-answer-reduction.tex:L200-L205`. |
| `lambda` (fixed-point context) | The resource-bound parameter of `def:lambda`, with term sort `P{Nat}` inside `Psi_{M,lambda}`; it is supplied to `Compress` and to `FuelBound(n, lambda)`; the code name is `lambda` (DESIGN §1.1; brief 24). | `gt-05-games-normalform.tex:L641-L653` (`def:lambda`). |
...
| `SOURCE_REPAIR` | A totalizing convention or typo repair needed to make the executable interpretation definite, or a construction change that closes a source gap, with the changed theorem input named. |
...
| `TypedSamplerDescription` | Refinement alias for a `SamplerDescription` whose `typing` field is `Typed(TypeSet,TypeGraph)`; it is not a second representation. | `gt-06-types.tex:L95-L140` (`def:typed-sampler`); DESIGN §§9.1, 9.6. |
...
| `TypedDeciderDescription` | Refinement alias for a `DeciderDescription` whose `typing` field is `Typed(TypeSet)`; it shares the same quote and metadata representation. | `gt-06-types.tex:L185-L195` (`def:typed-decider`); DESIGN §§9.1, 9.6. |
| `VerifierDescription` | Canonical pair of a field-aligned `SamplerDescription` and `DeciderDescription`, plus the normal-form and construction certificates consumed by transformation signatures. | `gt-05-games-normalform.tex:L624-L634` (`def:normal-ver`); DESIGN §9.6. |
| `QuotedLaw{Nat}`, `UpperBoundLaw`, `LawCert` | Closed law program and its replay certificate. AST equality and finite evaluations are CHECKED; a sampler-producing certificate also replays `enu:cl-space-sum` and `enu:cl-map-sum`. An unspecified theorem polynomial stays opaque and CITED. This is separate from `BoundExpr`. | `gt-04-cl.tex:L151-L180` (`lem:cl-kth`); project v2 representation for `gt-12-compression.tex:L156-L187` (`tab:params`). |
| `GapMap` | Canonical directed threshold relation for detyping, anchoring, introspection, answer reduction, repetition, or compression. `IntroGap` contains the explicit maximum of the transformed-input Ent term and `(1-delta)2^(2^(lambda*n))`. Only an actually evaluated branch may be CHECKED; every semantic implication stays CITED. | `gt-06-types.tex:L454-L460`; `gt-08-introspection.tex:L798-L815`; `gt-11-parallel-repetition.tex:L243-L258`; `gt-12-compression.tex:L40-L52`. |
...
| `anchored_repeat(v,lambda,tau)` | Folded spelling of public `repeat(v,lambda,tau)` on the pre-anchoring verifier: anchor and detype once, then take the repeated direct sum. Its output has level `ell+2` and dimension `k(n)(s(n)+8)`; internal `repeat_sampler` consumes an already anchored sampler and adds no level. | `gt-11-parallel-repetition.tex:L80-L258`; DESIGN §§9.4, 10.2. |
...
| `IntroAnswerEncoding` | The source's wire format serializes every non-Pauli vector field as a full `Q_EPR`-bit vector, zero outside the first `s(N)` coordinates; it requires `Q_EPR>=s(N)`. TB7 violates that embedding and prints the affected schemas `VACUOUS(owner=Q_I<s_0)`. | `gt-08-introspection.tex:L524-L530`, `L588-L591`; DESIGN §§11.4, 12.5. |
...
| `typed_intro_decider` | Executable seven-input predicate implementing Pauli, sampling, hiding, original-game, and equal-type consistency tests at child index `N=2^n`. Production times out each child at `N^lambda`; an explicit toy fuel substitution is recorded by the failed `toy_child_fuel` predicate. | `gt-08-introspection.tex:L394-L692` (`fig:intro-decider`); DESIGN §§11.4, 12.4. |
| `TIME_child(N)`, `F_child`, `toy_child_fuel` | Child runtime, supplied per-call fuel, and production equality `F_child=N^lambda`, in the metered quoted-interpreter unit specified in DESIGN §11.4. TB6b supplies 65,536 steps while retaining source budgets 4/16, so `toy_child_fuel=FAIL(owner=tb6-child-meter)`; exact honest costs remain NOT_EVALUABLE until traced, independently of description-size and embedding predicates. | `gt-08-introspection.tex:L417-L419`, `L983-L988`; DESIGN §§11.1, 11.4, 11.6, 12.4. |
...
| `ProductionPolicy`, `ToyPolicy` | Production retains exact symbolic paper laws and refuses over-budget materialization; toy mode supplies explicit parameter/count/fuel overrides and reports every violated production predicate, making theorem eligibility false. Every paper-parameter guard is evaluated against its honest toy witness with the outcome and owner visible, including FAIL/VACUOUS consequences. | Project v2 execution policy motivated by the growth in `gt-12-compression.tex:L15-L24`, `L156-L187`; DESIGN §12.4/DD-31. |
| `P_pcp_encodes_D1` | Content-fidelity predicate: the PCP instance passed to `pcpverifier` is the arithmetized bounded trace of the actual fixed-width introspection decider `D1` at the printed `(T,sigma_1)`. Parameter-shape equality alone cannot satisfy it. | `gt-10-answer-reduction.tex:L1813-L1828`, `L1958-L1960`, `L2060-L2063` (`enu:ar-game`); DESIGN §§12.4–12.5. |
...
| `SOURCE_REPAIR(zero-map-factor-partition)` | A whole-space zero CL map promoted to level `ell>=1` reports the whole ambient factor at stage 1 and empty factors thereafter. This replaces literal all-zero factor reports so `enu:cl-space-sum` holds. Originating cases are Anchor, PauliX/PauliZ, new introspection types, and detyping's zero conditional child. | `gt-04-cl.tex:L122-L130` (`rk:higher-level`), `L151-L180` (`lem:cl-kth`); source conflicts at `gt-11-parallel-repetition.tex:L96`, `gt-07-ldt.tex:L1106-L1108`, `gt-08-introspection.tex:L333-L345`. |
| `SOURCE_REPAIR(intro-3Q-guard)` | The executable introspection decider rejects answers longer than `3Q` (`>3Q`), accepting an honest three-vector Hide tuple of exactly `3Q`. The paper's literal `>=3Q` predicate remains printed and separately graded. | Literal guard: `gt-08-introspection.tex:L424-L425`; honest maximum: `L588-L591`; DESIGN §11.4. |
| `SOURCE_REPAIR(intro-decider-fixed-width)` | Canonically pads the embedded sampler, decider, and parameter slots of `D1` to fixed widths determined by `(lambda,ell)`, making `sigma_1` and the repaired compressed-sampler bytes independent of input-verifier bytes. The paper's length-bound-to-length-equality step remains CITED as a gap. | `gt-08-introspection.tex:L757-L776`; `gt-10-answer-reduction.tex:L2094-L2096`; `gt-12-compression.tex:L128-L147`; DESIGN §12.3. |
| `SOURCE_REPAIR(HaltDeciderFuelBound)` | The halting decider `D_{M,lambda}` runs the RETURNED compressed decider under `Eval(..., FuelBound(n, lambda))`, an enforced budget of `n^lambda` units; below it the fixed point returns `OutOfFuel`, which is not a decider answer. This is a construction change: `fig:halt_f` step 5 accepts iff `D^compr` accepts `(n,x,y,a,b)` with no budget, and `TIME_{D^halt}(n) <= n^lambda` is `lem:lambda`'s CONCLUSION for `lambda >= ceil(max{lambda_0, lambda_1, lambda_2})`, not a specification. The `lambda_bounded_time` hypothesis on `V = (S_lambda, D_{M,lambda})` therefore reads "budget enforced by construction, not measured"; on the two-state fixtures at `n = 2` the delimiter never binds (12 and 91 units), so every tested outcome coincides with the unbudgeted reading (`verdicts/tb4-r1.md` O4). | `gt-12-compression.tex:L448-L449` (`fig:halt_f` step 5, "Accepts if the decider ... accepts (n,x,y,a,b)"; `\label{fig:halt_f}` at L455); `L570-L638` (`lem:lambda`). |
| `M_loop` | Two-state toy machine whose halt state is unreachable, used to exercise the nonhalting branch and one finite description-level unfold of `D_{M,lambda}`. | Project TB7 fixture for `gt-12-compression.tex:L426-L492` (`fig:halt_f`). |
