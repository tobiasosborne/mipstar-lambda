...
| `Level` | Finite serializable compression-level data. The fixed-point parameter `L` is a closed literal term of sort `P{Level}` and is passed unchanged to `Compress`; the fuel bound `FuelBound(n,lambda)` takes the resource bound `lambda : P{Nat}` of `def:lambda` (DESIGN 1.1), not a CL level. | Mandate `handoff.md:L21-L35`; compression interface `gt-12-compression.tex:L26-L39`. |
...
| `L` (fixed-point context) | The compression-level parameter, with term sort `P{Level}` inside `Psi_{M,L}`; it is supplied to `Compress`, while `FuelBound` takes `lambda : P{Nat}` (see `Fuel`). | Mandate `handoff.md:L21-L35`; compression interface `gt-12-compression.tex:L26-L39`. |
