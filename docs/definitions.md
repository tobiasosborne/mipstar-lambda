...
| `IntroAnswerEncoding` | The source's wire format serializes every non-Pauli vector field as a full `Q_EPR`-bit vector, zero outside the first `s(N)` coordinates; it requires `Q_EPR>=s(N)`. TB7 violates that embedding and prints the affected schemas `VACUOUS(owner=Q_I<s_0)`. | `gt-08-introspection.tex:L525-L531`, `L588-L591`; DESIGN §§11.4, 12.5. |
