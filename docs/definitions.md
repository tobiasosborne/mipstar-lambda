...
| `Level` | Finite serializable compression-level data (a CL level, `level(V)`). The fixed-point parameter is NOT a level: it is `lambda : P{Nat}`, the resource bound of `def:lambda` (`gt-05-games-normalform.tex:L641-L653`), passed unchanged to `Compress` (DESIGN §1.1, renamed 2026-09-05 per `verdicts/analytic-doc-r1.md` M1). |
