...
**Nested typed deciders (brief 80 D12; brief 44 obligation (2); corrected per verdicts/tb6-r2.md N3).** A detyped introspection `TypedDecider` child reached through `(:Detype, ...)` charges input decoding on the enclosing meter and each child call on a meter bounded by `min(own budget, enclosing remaining)`. TB7 additionally charges type-label scans and a fixed predicate reservation at depth `+1`, and threads the parent meter into nested `:Pauli` and `:AnswerReduce` bodies. On the TB6b-E game edge the unbudgeted result is `(22747, [22618, 129])`: depth 2 is 114 reserved own steps plus 15 child-call steps; one unit short rejects at `(22745, [22618, 127])`. The three direct TB7 nested-body budget tests are red/green. **Gap, UNCHARGED(owner=tb7-nested-own-steps):** vector projections, dual-map elimination, Pauli field algebra, and AnswerReduce low-degree/PCP arithmetic are not yet charged at their individual §11.4 operation counts. The reservation detects an enclosing budget too small for a nested body's entry and guard work, but it is not a complete step account for those computations. Consequently the general `gt-08:L417-L419` child-time claim and any `D_{M,lambda}` fuel claim on a transcript reaching such arithmetic retain this owner; the finite TB7 anchor transcript and separate synthetic evaluator-entry test do not reach those bodies.
