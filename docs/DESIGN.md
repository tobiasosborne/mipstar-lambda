...
5.67 s / ratio 37.5 quiet before it and QUIET_BODY s / ratio QUIET_RATIO after it (QUIET_RUNS quiet runs, ratio range QUIET_RANGE), against the
...
Thus `M5-anchor-zero`, `M5-anchor-answer`, `M5-or`, and the strict component boundary each have a named red witness.  The construction, the 128 positive plus negative transcripts, and their total are gated as calibrated ratios against the suite kernel (§13.1; `CALIBRATED_GATES` K = 4, 4, 6, ceilings 7.68 s, 7.68 s, 11.52 s; the pre-implementation targets `<2 s`/`<5 s`/`<7 s` were replaced by brief 80 D2), and peak allocation is asserted `<256 MiB`.
...
**The fuel unit, as charged (`verdicts/tb6-r1.md` §4, brief 80 D8).** One unit is one primitive step of the universal interpreter machine executing a description, counted by `Meter.steps`, and DEFINED by the following exhaustive table of charge sites (`src/descriptions/machines.jl` `_validated_answer` L779-L811 and the composite machines; `src/introspect/meter.jl`; corrected per `verdicts/tb6-r2.md` N7 -- the earlier prose list omitted the accumulate, walk, scan, indicator, type-label and elimination-element charges and did not reproduce the pinned counts):

| site | charge |
|---|---|
| query header (every mode) | `1 + ndigits(n; base=2)` (4 at `N=4`) |
| Dimension answer | `1` (a leaf's `_dimension` charges nothing; composites add their child calls) |
| (player, stage) fields | `2` |
| typed label | `ncodeunits(type)` |
| input vectors | `s` (Marginal seed, Factor prefix), `2s` (Linear prefix + input) |
| metered stage of factor size `k` | `k` reads + `k^2` multiply-accumulates + `k` writes + `1` branch selection |
| Marginal accumulate, per evaluated stage | `k` |
| prefix walk, per walked stage | `k` key reads + `1` branch selection (+ the column-space check when the walk is `reachable`, i.e. for Factor) |
| prefix walk, final support scan | `s` |
| column-space check `[A|b]`, `A` of size `rows x cols` | `rows(cols+1)` copy; per pivot column `rows - p + 1` search; `2(cols+1)` per swap; `cols+1` scale; `cols+1` per eliminated row; `rows(cols+1)` final scan |
| Factor indicator | `s` |
| Linear on the selected stage | `k + k^2 + k` (no branch) |
| output serialization | `length(answer)` |
| control transfer to a child machine (`_forward`) | `1` |
| composite machines' own register work | as charged at each `_charge!` in `machines.jl` (graph registers `4|Type|+|z|`, revealed-type scan `2|Type|`, prefix scans `|u|`/`2|u|`/`3|u|`, zero maps `s`, concatenation `length(out)`) |

Worked derivation of the `TB6b-M` charge table from this table (alice, untyped, `s=6`, stage factor sizes `1, 2, 3` on the `e_1` branch; TB6b (i) re-derives all four in the test): `Dimension = 4 + 1 = 5`; `Marginal(3) = 4 + 2 + 6 + (4+1) + (9+2) + (16+3) + 6 = 53`; `Factor(2,e_1) = 4 + 2 + 6 + [1 + 7 + 1] + 6 + 6 + 6 = 39` (the elimination of `[1|1]` costs `2+1+2+2 = 7`); `Linear(2,e_1,e_4) = 4 + 2 + 12 + [1 + 1] + 6 + (2+4+2) + 6 = 40`.  A control transfer is one step and `by_depth` attributes every step to its machine depth.  The budget refuses the `(budget+1)`-th step BEFORE it executes (a return at step `budget` is permitted); the thrown `FuelExhausted` reports the ATTEMPTED count of the refused block, never a step index, and `Meter.steps` is unchanged by a refused block (`verdicts/tb6-r1.md` O9).  "It first asks only `Dimension(N)`" describes the decider's FIRST call, not a bound on the schedule: every vector query runs the child's own sizing `Dimension` probe on the same meter (7 child `Dimension` calls for 6 vector queries on the Hide_1/Hide_2 leaf), which the query schedule below excludes and the decider's trace records as one `Dimension` record (`verdicts/tb6-r1.md` O8).

**Fuel currency (brief 80 D11; brief 44 obligation (1)).** There is ONE currency: an `Eval` fuel unit of §1.1 IS a `Meter` step, and the charge list above is the definition of that unit.  A description runs under `Eval` through the primitives `sampler_machine`/`decider_machine` (`src/compress/lowering.jl`): the primitive runs the universal description interpreter on the quoted bytes with the REMAINING fuel as its step budget and charges exactly the metered steps, so `used(Eval) - steps(Meter)` is a constant program overhead across queries (CHECKED, TB6b (m)); a quoted §1.1 program as a decider child (`(:Program, bytes)`) runs on the CEK evaluator with the enclosing meter's remaining budget as its fuel and its `OutOfFuel` is the child's timeout.  This is what makes `TIME_{D^halt}(n) <= n^lambda` of `SOURCE_REPAIR(HaltDeciderFuelBound)` and the per-call `R` gate of `gt-08-introspection.tex:L417-L419` the same quantity; `fig:halt_f` step 5 (`gt-12-compression.tex:L448-L449`) and Bounded Halting (`gt-10-answer-reduction.tex:L200-L205`) spend it.

**Nested typed deciders (brief 80 D12; brief 44 obligation (2); corrected per verdicts/tb6-r2.md N3).** A detyped introspection `TypedDecider` child reached through `(:Detype, ...)` is metered, not refused, but only partly: its input decoding is charged on the enclosing meter, and every child call it makes runs on its own meter with budget `min(own budget, the enclosing meter's remaining budget)` and is charged to the enclosing meter at depth `+1` when it returns or times out (the steps a timed-out call executed included: `(22631, [22618, 13])` at budget 22,632 on the TB6b-E game edge, `M6-nested-timeout-uncharged` red); an exhausted enclosing budget refuses the call before its first step (a timeout, hence a rejection).  The nested verdict equals the flat verdict and the depth-attributed steps sum to the total (CHECKED, TB6b (k); `M6-nested-depth` red).  **Gap, UNCHARGED(owner=tb7-nested-own-steps):** the nested decider's OWN predicate work (register projections, vector comparisons, the dual-map elimination) is not charged — its depth-2 steps equal the sum of its child calls' steps exactly (15 = 5 + 10 on that edge, pinned) — and a nested `:Pauli` or `:AnswerReduce` body receives no parent meter at all (its whole check is charged `2+|x|+|y|+|a|+|b|`).  So gt-08:L417-L419's bound on the subroutine's whole running time is enforced on child calls and input decoding only; brief 44 owns the fix (meter the nested own work in the §11.4 unit at depth `+1`, thread `parent` into the `:Pauli` and `:AnswerReduce` bodies, red test: a nested body whose own work alone exceeds the enclosing budget times out).

The explicitly ineligible TB6b policy in §11.6 instead supplies `F_child=65,536` to that same counter and prints `toy_child_fuel=FAIL(owner=tb6-child-meter)` for the failed production equality `F_child=R`.  No production-fuel acceptance follows from such a run.  Each child-call record must include fixture, quote hash, mode, role, stage/prefix/input, exact metered cost, source `R`, supplied `F_child`, and timeout/return; the per-call costs are measured exactly and the ten per-mode maxima are pinned (§11.6), while the production budget fit `TIME_child(N)<=R` is graded FAIL/PASS per slot as measured and the general source TIME bound stays `NOT_EVALUABLE(owner=tb6-child-meter)`.  The finite per-mode maximum is computed from those records, not substituted for the general source TIME bound.
...
TB6a is the design audit: instantiate the finite type/edge formulas for `ell in {1,3,9}`, generate every parser schema and guard, and compare the emitted query plan with the source tables.  It performs no quantum claim.  Its wall is gated by the calibrated ratio of §13.1 (`K_tb6a = 18` kernel units, quiet in-suite measurement 2.765 s = 5.76 kernels; the pre-implementation `<1 s` target is missed by first-use compilation of the Pauli maps and guards, `verdicts/tb6-r1.md` O2/O6).
...
For every oriented edge, enumerate every nonzero-support stabilizer outcome with an exact dyadic probability (structural: every leaf of the complete binary tape tree carries `2^-depth`, so "total mass one" is an identity of the enumerator and is NOT an assertion, `verdicts/tb6-r1.md` O3); the checks that can fail are the exact leaf count (14,378), the aggregate leaf-probability histogram and the SET of distinct per-pair leaf counts `{1,2,72,88,120,256}` (pinned; not a per-pair map), and the agreement, on every (edge, seed), of the tableau's leaf distribution with a TEST-ONLY dense 64-amplitude reference simulator run through the same honest strategy -- the prohibition of a dense state vector below binds the PRODUCTION simulator only.  Acceptance probability exactly one is a target only for the **operative toy** decider supplied with `F_child`, after every honest child cost has been measured and shown to fit that budget; an absent trace or a timeout prevents an acceptance PASS.  Acceptance under the source `R=4` gate is withdrawn.  The report separately evaluates
...
tree by every valid graph encoding plus its unconditional invalid-encoding branch, then run 256 seeded draws as a secondary distribution regression.  Also require the per-call fuel records specified below, sampler level 5, dimension 142, and stable bytes.  With `|a|=1`, a Read answer has `2Q+1=5<6=3Q`, so no Read pair adds to the literal rejection count 10.  Construction plus transcripts are gated as the calibrated ratio `tb6b_E` (K = 33, ceiling 63.36 s; measured in-suite 8.5 s on the 4-core container; the pre-implementation targets `<3 s`/`<15 s` were replaced by brief 80 D2), peak `<512 MiB`; a dense state vector is forbidden in the production simulator.
...
`>=1/2` are printed `VACUOUS`, never `PASS`; hence `enu:hiding-same` and the low-degree checks are `VACUOUS` on `TB6b-E` but live on `TB6b-M`.  E is therefore a degenerate hiding fixture by construction (`ell=1`, `s(N)=1`, `V_{>1}` empty): all live adaptive hiding evidence -- prefix-dependent factors, the dual map, both `intro-*` source repairs -- rests on the single `TB6b-M` child sampler (`verdicts/tb6-r1.md` O10).  Every conjunct of items 2(a)-3(d) of the predicate (thirteen comparisons) has a NAMED negative transcript: an honest leaf of its oriented pair with exactly the compared field corrupted, rejected with the fired-test name, and a conjunct-drop mutant per comparison (`verdicts/tb6-r1.md` O1; TB6b (j)).  Construction plus transcripts are gated as the calibrated ratio `tb6b_M` (K = 21, ceiling 40.32 s; measured in-suite 5.3 s), E and M together as `tb6b_combined` (K = 54, ceiling 103.68 s), peak `<512 MiB` (brief 80 D2 replaced the pre-implementation estimates).
...
The honest-child cost report has the following required slots, MEASURED (the finite maximum of the exact metered cost over the recorded honest child calls, in the unit of §11.4; TB6b (e)/(i), pinned exactly, `verdicts/tb6-r1.md` §1 and O4).  `NE` would expand to `NOT_EVALUABLE(owner=tb6-child-meter)`, not zero, a runtime bound, or a PASS; no slot is `NE` any more.

| fixture | child mode | source `R` | supplied `F_child` | exact honest cost (steps) | `TIME_child(N)<=R` | finite cost fits `F_child` |
|---|---|---:|---:|---:|---|---|
| TB6b-E | Dimension | 4 | 65,536 | 5 | FAIL (5 > 4) | PASS |
| TB6b-E | Marginal | 4 | 65,536 | 13 | FAIL | PASS |
| TB6b-E | Factor | 4 | 65,536 | 10 | FAIL | PASS |
| TB6b-E | Linear | 4 | 65,536 | 13 | FAIL | PASS |
| TB6b-E | child decider | 4 | 65,536 | 10 | FAIL | PASS |
| TB6b-M | Dimension | 16 | 65,536 | 5 | PASS | PASS |
| TB6b-M | Marginal | 16 | 65,536 | 53 | FAIL | PASS |
| TB6b-M | Factor | 16 | 65,536 | 60 | FAIL | PASS |
| TB6b-M | Linear | 16 | 65,536 | 50 | FAIL | PASS |
| TB6b-M | child decider | 16 | 65,536 | 22 | FAIL | PASS |

The charge table on the `TB6b-M` child at `z*` (one call per mode, TB6b (i), pinned): `Dimension` 5, `Marginal(3)` 53, `Factor(2,e1)` 39, `Linear(2,e1,e4)` 40, `Factor(3)` 60, `Linear(3)` 50.  The raw mode/role/stage/input trace is retained; a timed-out run prints `FAIL` with the consumed budget rather than as a completed honest cost.  Only measured fit to `F_child` can enable the corresponding operative-toy acceptance check.  The general source TIME predicate still needs its own discharge and never inherits a toy PASS.  Owner `tb6-child-meter`, tracked by `mipstar-lambda-9w7`, retains the stronger production-fuel acceptance and exact-cost obligation for C14's later adjudication.  C14 is TESTED (scoped; `verdicts/tb6-r1.md` §5, row replaced by `verdicts/tb6-r2.md` §6); the production-fuel obligation above is not part of it.
...
**Padding cost and the `(:Pad, extra, S)` node (verdicts/tb5-r1.md O12; verdicts/tb6-r1.md O7; brief 80 D7/D13).** Padding a level-`r` sampler to a higher level inside a direct sum decides `u in L_{<=r}(V)` at the parent by rebuilding the child's column spaces through its queries (`_require_image`), a super-linear cost: MEASURED at TB6a (3), in the §11.4 unit, `74,671` steps / 148 child calls at dimension 142, `113,946` / 185 at 179, and `155,631` / `212` at TB7's own introspection block (`ell=9`, dimension 206; fitted exponent `1.97` over the single blocks 142/179/206), `0.0004` s of wall at 206 and, for the eight-block direct-sum PROXY of TB7's 1696 (1648 = 8·206; TB7's 848/1696 themselves are not measured), `17,899,098` steps / `1,654` child calls (pinned, as are 206 and the four-block proxy 824: `4,488,710` / `830`; child calls = dimension + 6 at every size) and `0.0349` s of wall (advisory; the r2 critic measured `0.0677` s) (1648; the TB7 chain is `206 -> 840 -> 848 -> 1696`, and a k-fold direct sum rebuilds each summand separately).  Decision procedure: the node is a REQUIRED TB7 obligation iff the projected per-construction cost exceeds 1 s or one tenth of TB7's 60 s warm target divided by the constructions on the TB7 path; outcome (one in-suite run, 2026-09-22, 4-core cloud container, kernel 0.7825 s): **not binding at TB7 scale** -- the 1648 proxy costs < 0.07 s, far below both thresholds -- so the node is NOT a required TB7 obligation on cost grounds; the TB7 tree implements it anyway (below), and `_require_image` is retained.  The explicit node `(:Pad, extra, S)` (`PadMachine`, `src/descriptions/machines.jl`) admits stage `level+1` internally as the empty stage after the last, so a padding parent decides the prefix through the CHILD's own walk instead of rebuilding column spaces; `_require_image` stays in place only for the implicit padding of `DirectSum`/`Product` children, and the public boundary still refuses `j > level`.
...
|---|---|---|---:|---|
| TB5 Repeat | `V_copy`; `n=9,lambda=tau=1,c'=1,k=81`; dimensions `1->9->729` | four-query replay, 128 classical honest accepts, four named negative/boundary transcripts, sampler hash | calibrated ratio gates (K 4/4/6 kernel units; ceilings 7.68/7.68/11.52 s), `<256 MiB` | seven `M5-*` in §10.3 |
| TB6a design audit | `ell=1,3,9`; type/edge and parser generation | counts, schemas, source-query plan, no theorem claim | calibrated `K=18` (quiet 2.765 s in-suite; the `<1 s` estimate is missed by first-use compilation) | missing edge, wrong count, missing guard; `M6a-gate-body-inflated` |
| TB6b Introspect | `TB6b-E`: `n=2,N=4,lambda=1,R=4,ell=1,(q,m,d)=(2,1,1)`, dimension 142; `TB6b-M`: `lambda=2,R=16,ell=3,s=6,(8,2,1)`, dimension 179; both `F_child=65,536` | E: exact support over 116 pairs (14,378 leaves, exact dyadic probabilities, leaf count and probability multiset pinned, dense-reference agreement); acceptance only after measured fit to toy fuel, literal rejection count 10; M: live adaptive factor/dual/game checks, `T6-view-swap`, 512 draws, literal rejection count 22; thirteen named negative transcripts; per-mode costs pinned, `toy_child_fuel=FAIL(owner=tb6-child-meter)`, VACUOUS grades | calibrated: E (construction + transcripts), M, combined, each `K` set from the quiet ratio; `<512 MiB` | twelve mutations in §11.6, the thirteen conjunct drops, `M6b-gate-body-inflated` |
...
**Clock calibration (verdicts/tb5-r1.md §4 condition (ii); verdicts/tb6-r1.md O2; brief 80 D2).** Every wall gate of the suite is a RATIO gate against `suite_calibration_kernel()` (`test/calibration.jl`): a deterministic, in-process kernel -- 1,200,000 passes over the 512 ordered `GF(8)` triples accumulating `a*b + c*e_i` over runtime data, measured quiet at 0.48 s on the reference box (performance governor, 4.8 GHz) -- timed once BEFORE any timed body and therefore excluded from every gated region.  A gate is `elapsed / kernel < K` with `K = max(4, ceil(3 * quiet_ratio))` (3x headroom over the quiet in-suite ratio; the ratio is load-invariant where an absolute wall is not: the mutation runner's own 4-way load slowed the kernel 2.7x) plus an absolute ceiling `4 K * 0.48 s` (about 7.4-8x the in-suite measurement on the 4-core container for the TB5/TB6b rows, since there `K = max(4, ceil(3 * ratio))` rounds up little; at least 12x only where the kernel runs at the reference box's 0.48 s) that the runner's load should not reach; each gate prints its ratio and `K * kernel`, the budget enforced at that run's own rate.  TB0: `K=50`, ceiling 60 s; TB4: `floor(6 s / kernel_reference)`; TB5, TB6a, TB6b: the `CALIBRATED_GATES` table of `test/calibration.jl`.  Red witnesses inflate a BODY, never the kernel (`M-gate-body-inflated`, `M6a-`/`M6b-gate-body-inflated`).
