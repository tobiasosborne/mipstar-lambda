...
**Nested typed deciders (brief 80 D12; brief 44 obligation (2)).** A `TypedDecider` child (a detyped introspection or answer-reduced decider reached through `(:Detype, ...)`) is metered, not refused: its input decoding is charged on the enclosing meter, and every child call it makes runs on its own meter with budget `min(own budget, the enclosing meter's remaining budget)` and is charged to the enclosing meter at depth `+1` as it returns; an exhausted enclosing budget refuses the call before its first step (a timeout, hence a rejection).  Hence no level ever executes a step the enclosing budget could not pay for, the nested verdict equals the flat verdict, and the depth-attributed steps sum to the total (CHECKED, TB6b (k); `M6-nested-depth` red).
