...
\caption{Layer 1 is split: the green box is the identity evaluation the prototype performs, the dashed box beside it is the conditional theorem that turns that evaluation into an accepting computation, and only the second carries C5.  The arrows are the real dependency --- layers 2 and 3 feed layer 1, and layers 1 and 4 feed \code{thm:ar} --- not a top-to-bottom chain.}
...
A claim column reading ``none'' or ``no current row'' means exactly that: no
row of \code{claims/CLAIMS.md} governs the object.  In particular every
theorem and lemma of Part~II is a written derivation of this project at
\textsc{SKETCH --- not yet adversarially verified} status, with no ratchet
row, as stated at the head of Section~\ref{sec:resource-lambda}.
...
\caption{The paper's bounded decider passes through quoted and succinct IR sorts toward three named front-end operations that are still absent.  The dashed final arrow marks this row as a cited contract rather than an executed construction; the Part~II theorems it rests on are \textsc{sketch --- not yet adversarially verified} and carry no ratchet row.}
...
\caption{The 18-type sampler and guarded answer-reduced decider have constructed or finite-checked shapes, carrying C4b and C9 at \textsc{Tested}.  Detyping and the quantum implication in \texttt{thm:ar} stay dashed: detyping has no ratchet row and \texttt{thm:ar} is only C5 \textsc{Sketch}.}
