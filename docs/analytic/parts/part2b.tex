...
\caption{Both Schwartz--Zippel bounds must fall left of the amber acceptance threshold, which is exactly what the choice \(q=2^k\) buys.  The two bounds are cited analytic input; only the parameter arithmetic that compares them with \(1/2\) is executed here.}
...
\caption{The amber seed coordinate \(s=5\) falls in the second of the two \(q/m=4\)-element buckets, so \(\chi(s)=2\) and the axis-line map takes its direction \(e_2\) from a stage that reads \(s\) alone.  The second stage is then linear on \(u\) with the choice already fixed, which is exactly why \(L_{\rm ALine}\) is conditionally linear of level two rather than one.}
