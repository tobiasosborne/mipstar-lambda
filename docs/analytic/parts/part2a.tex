...
\caption{Every named result of Part~II belongs to exactly one of three columns: transcribed from the paper, derived here and unverified, or executed and ratcheted.  The middle column is the largest, which is the honest shape of a document that costs a construction rather than proving its soundness.}
...
adversarially verified and carry \emph{no} row in \code{claims/CLAIMS.md}.
Their status is \textsc{SKETCH --- not yet adversarially verified}, and the
...
\(c_Y=3\) for the self-code link.  Third, \S1.1 fixes the hole sort to
\(\mathsf{Quoted}\{\mathsf{Decider}\}\), whereas
Section~\ref{sec:resource-lambda} admits \(\mathsf{Quoted}(A)\) for any result
sort \(A\).  All three are strengthenings, not simplifications; each is a
proposed edit to \code{DESIGN.md} \S1.1, so that the two documents move in
lockstep rather than silently diverging.
