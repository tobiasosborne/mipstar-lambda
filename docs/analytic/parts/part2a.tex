...
\((n,x,y,a,b)\), where \(n\) is an integer in binary and
...
Here \(\operatorname{level}(S)\) is the nesting depth of the sampler as a
conditionally linear function; the inductive definition is
Definition~\ref{def:cl-func-doc} in Section~\ref{sec:typed-layer}, and until
then only the integer matters.  These are \code{def:normal-ver} and
\code{def:lambda}.
...
Four further symbols travel with \(\lambda\) through the whole of Part~II and
are collected once, in Figure~\ref{fig:parameter-card}, rather than introduced
where they are first used.  They are the constants \(\Compress\) is
parameterized by; the toy values at which the project instantiates them are in
Figure~\ref{fig:tb7-card}.\gt{fig:compress, eq:mu-gamma, eq:c\_rep}{ground-truth/gt-12-compression.tex}
\begin{figure}[H]
\centering
\input{figs/fig-parameter-card.tex}
\caption{The eight constants \(\Compress\) carries, each with the ground-truth label that fixes it, so that \(\mu,\gamma,\tau\) and \(k(n)\) are defined before they are used in Sections~\ref{sec:self-reference} and~\ref{sec:tables}.  Reading them together also shows that only \(\lambda\) is supplied per call; the rest are decided once and for all.}
\label{fig:parameter-card}
\end{figure}
...
\item \textbf{Answer reduction.}  In Step 1 of \code{fig:pcpverifier}, which
the answer-reduced decider invokes from its game-check step, the verifier
computes
\[
...
The construction uses \(|D|\leq\sigma\) quantitatively.\gt{fig:pcpverifier, prop:standard-succinct-sat}{ground-truth/gt-10-answer-reduction.tex}
Figure~\ref{fig:ar-invokes-pcp} separates the two machines that this one step
belongs to.
\begin{figure}[H]
\centering
\input{figs/fig-ar-invokes-pcp.tex}
\caption{The answer-reduced decider owns the five guards of \code{fig:decider-pcp}; only its last guard calls \code{pcpverifier}, and Step 1 --- the succinct-circuit construction from \(\overline D\) --- lives inside that call.  Keeping the two apart is what makes the description, rather than the tableau, the object the compiler handles.}
\label{fig:ar-invokes-pcp}
\end{figure}
...
Figure~\ref{fig:three-provenances} sorts every named result of Part~II into
three provenances, and the reminder below states the consequence for the
middle column.
\begin{figure}[H]
\centering
\input{figs/fig-three-provenances.tex}
\caption{Every named result of Part~II belongs to exactly one of three columns: transcribed from the paper, derived here and carried at \textsc{sketch} by \texttt{C16}--\texttt{C19}, or executed and ratcheted by a converged verdict.  The middle column is the largest, which is the honest shape of a document that costs a construction rather than proving its soundness.}
\label{fig:three-provenances}
\end{figure}

\begin{reminder}
\textbf{Status of everything typeset as a theorem from here to
Section~\ref{sec:tables}.}  Sections~\ref{sec:resource-lambda}--%
\ref{sec:l-cook-levin} state results about \(\mathcal L\) --- the language
defined here --- and not about the paper.  They are this project's own
derivations: Theorems~\ref{thm:l-determinism}, \ref{thm:fuel-monotonicity},
\ref{thm:quote-eval}, \ref{thm:tm-to-l}, \ref{thm:l-to-tm},
\ref{thm:lambda-preservation}, \ref{thm:kleene-two}, \ref{thm:ycode},
\ref{thm:fixed-point-equivalence} and \ref{thm:l-succinct-sat}, and
Lemmas~\ref{lem:specialization}, \ref{lem:smn} and
\ref{lem:transition-window}, are written proofs which have \emph{not} been
adversarially verified.  They are recorded as \code{C16}--\code{C19} in
\code{claims/CLAIMS.md} at \textsc{SKETCH --- not yet adversarially
verified}: a written derivation, no machine check, no converged verdict.  The
four rows partition exactly this list --- \code{C16} the semantics and fuel
results of Section~\ref{sec:resource-lambda}
(Theorems~\ref{thm:l-determinism}--\ref{thm:quote-eval},
Lemma~\ref{lem:specialization}), \code{C17} the costed machine/term bridge of
Section~\ref{sec:simulation} (Theorems~\ref{thm:tm-to-l}--%
\ref{thm:lambda-preservation}), \code{C18} the self-reference results of
Section~\ref{sec:self-reference} (Lemma~\ref{lem:smn},
Theorems~\ref{thm:kleene-two}--\ref{thm:fixed-point-equivalence}), and
\code{C19} the Cook--Levin construction of
Section~\ref{sec:l-cook-levin} (Lemma~\ref{lem:transition-window},
Theorem~\ref{thm:l-succinct-sat}).  The
constants they name (\(a_0,b_0,c_0,a_U,b_U,C_L,c_Y=3,a_s,b_s,a_K,b_K\)) and
exponents (\(8\), \(3+\lceil\log_2 72c_0\rceil\)) are fixed by those proofs
alone.  Results transcribed from the paper are marked instead by a
ground-truth footnote and are \grade{CITED}; the two kinds are never mixed.
\end{reminder}
...
\subsection{Certificates and the derivation tree}
\label{sec:certificates}

Every evidence grade printed in Section~\ref{sec:tables} is a node of a
concrete data structure, not a marginal note.  The single source is
\code{docs/DESIGN.md} \S3; the implementation is
\code{src/certificates.jl}.  A transformation does not return a bare term; it
returns
\[
 \mathtt{Checked}\{T,C\}
 \;=\;
 (\,\mathtt{term}::T,\ \mathtt{certificate}::C\,),
\]
where \(C\) is a \code{CertNode}: a grade, a rule name, a finite record of
facts, an ordered tuple of children, and --- for one grade only --- a replay
function.  The grades are exactly five, and Figure~\ref{fig:grades} lists
them with what each obliges its author to supply.

\begin{figure}[H]
\centering
\input{figs/fig-grades.tex}
\caption{The five grades a \code{CertNode} may carry, in decreasing order of what they oblige the author to supply; only \grade{CHECKED} carries executable evidence, and only it is re-run at test time.  Reading them as a scale makes an all-\grade{CITED} subtree visibly worthless as evidence, however green the code around it looks.}
\label{fig:grades}
\end{figure}

\code{verify\_certificate} walks the tree and re-runs every
\grade{CHECKED} replay against the term actually attached; a
\grade{CHECKED} node with no replay function is a verification failure, not a
pass.  That is what makes stale evidence unrepresentable: there is no detached
cache to go out of date, and a mutation that changes the term but not the
certificate turns the suite red.  Symbolic bounds are either
\(\operatorname{Concrete}(k)\) or
\(\operatorname{Opaque}(\text{description},\text{parameters})\); no
\(\poly(\cdot)\) is silently given an exponent.

Figure~\ref{fig:certificate-tree} is the tree that
\code{AnswerReduce} actually builds on the TB2 fixture, exactly as
\code{test/tb2\_answer\_reduce.jl} inspects it.  Reading it top down answers the
question the grade columns of Section~\ref{sec:tables} raise: the
\emph{root} of the executed answer-reduction certificate is \grade{CITED},
because detyping is imported; the executed content sits strictly below it.
\begin{figure}[H]
\centering
\input{figs/fig-certificate-tree.tex}
\caption{The certificate that \code{AnswerReduce} returns on the TB2 fixture, with each node's grade, rule name and printed facts.  A \grade{CITED} root over \grade{CHECKED} children is the normal shape here, and it is why an executed subtree never promotes the theorem above it.}
\label{fig:certificate-tree}
\end{figure}

The TB5--TB7 design adds four \emph{predicate outcomes} on top of these
grades, for the parameter guards a toy instantiation cannot satisfy:
\code{PASS}, \code{FAIL}, \code{NOT\_EVALUABLE} (the cost or fit was never
measured) and \code{VACUOUS} (the guard set is empty, or a promised margin is
\(\geq1/2\)).  They are outcomes printed beside a predicate, not grades on a
node, and none of them may be folded into \code{PASS}; the TB7 report of
Figure~\ref{fig:tb7-card} is where they appear.
...
the transition has charge \(k\).  Every row is a full five-register
configuration \(\langle q,\eta,K,H,f\rangle\); in the application row
\(\eta'\) is the ambient environment at the call site, which is discarded in
favour of the closure's own \(\eta\) extended by the argument frame.
\[
\begin{array}{rcll}
...
\langle\operatorname{Apply}([\lambda r.t,\eta],\vec v),\eta',K,H,f\rangle
 &\longrightarrow_1&\langle t,(\vec v)::\eta,K,H,f-1\rangle
 &(|\vec v|=r),\\
\langle\operatorname{If}(1,v_1,v_0),\eta,K,H,f\rangle
 &\longrightarrow_1&\langle v_1,\eta,K,H,f-1\rangle,&\\
\langle\operatorname{If}(0,v_1,v_0),\eta,K,H,f\rangle
 &\longrightarrow_1&\langle v_0,\eta,K,H,f-1\rangle,&\\
\langle\operatorname{Prim}(p,B,\vec v),\eta,K,H,f\rangle
 &\longrightarrow_{\kappa_p(\vec v)}&
 \langle p(\vec v),\eta,K,H,f-\kappa_p(\vec v)\rangle,&\\
\langle\operatorname{Quote}(\operatorname{Closed}(t)),\eta,K,H,f\rangle
 &\longrightarrow_1&\langle\operatorname{Code}(t),\eta,K,H,f-1\rangle.&
\end{array}
...
\(\mathsf{TypeError}\) during that scan.  The constant \(3\) is not a spare
allowance: it pays for installing the Eval delimiter, removing it, and the one
tag check that decides whether \(d\) is well-formed code.  It therefore
\emph{absorbs} the two delimiter units charged above, and the two accounts do
not double-count.
...
Figure~\ref{fig:C-tmdelta-scan} zooms into the highlighted matching row of the
bounded table scan the next paragraph charges for.
\begin{figure}[H]
\centering
...
Figure~\ref{fig:u-l-heap} lays the records the proof builds onto the universal
evaluator's literal work tape and follows its lazy handle to a separate input
tape.
\begin{figure}[H]
\centering
...
\(C_L(|t|+f+1)^8\), independent of unread suffixes.  Hence a total decider term
with fuel schedule \(f_D(n)\) is a decider in the paper's sense and
\[
 \Time_D(n)\leq C_L(|t|+f_D(n)+1)^8.
...
length is at most \(c(|d|+|u|+f+1)\) for a universal \(c\).  Write
\(N=|d|+|u|+f+1\).  A conservative \emph{multitape} implementation finds a
record by a full scan of the heap track, compares binary addresses cell by
cell, and returns to the simulated head, all in at most \((cN)^2\) steps.

Figure~\ref{fig:exponent-ladder} is the whole cost ladder of this proof in one
column, with the point where the exponent doubles marked.
\begin{figure}[H]
\centering
\input{figs/fig-exponent-ladder.tex}
\caption{The five steps that produce \eqref{eq:l-to-tm-bound}, ending at the one-tape conversion that squares a multitape bound.  Making the doubling explicit is what keeps the statement and the proof of Theorem~\ref{thm:l-to-tm} in agreement.}
\label{fig:exponent-ladder}
\end{figure}

There are at most \(f\leq N\) microsteps, so the simulation itself costs at
most \(c^2N^3\); decoding the input serialization and writing the final answer
each take a constant number of passes over a track of length at most \(cN\),
which is absorbed.  Charged primitives run on their fixed reference machines
in at most \(c_p(1+\kappa_p+\sum_i|v_i|)^{e_p}\) steps, and
\(\kappa_p\leq f\leq N\) with \(\sum_i|v_i|\leq cN\), so each such step costs
at most \(c_p(c'N)^{e_p}\); since the registry is fixed when \(\mathcal L\) is
fixed, \(e_{\max}=\max_pe_p\) is a constant.  Taking
\(e_0=\max\{4,\,1+e_{\max}\}\) and increasing the constant, the whole
\emph{multitape} run is bounded by \(C\,N^{e_0}\).

Converting that fixed multitape machine to the paper's one-tape convention
costs the standard quadratic overhead, so the one-tape running time is at most
\(C_L\,N^{2e_0}\).  With the registry fixed here, \(e_0=4\) and the stated
exponent is \(8\); this proves \eqref{eq:l-to-tm-bound}.  The exponent is not
optimized and nothing downstream depends on its value: Theorem~\ref{thm:lambda-preservation}
and the \(A\)-rescaling use only the existence of \emph{some} fixed \(e\) and
\(C_L\) with \(\Time_{U_{\mathcal L}}(d,u,f)\leq C_L(|d|+|u|+f+1)^e\).  A
different primitive registry replaces \(8\) by \(2\max\{4,1+e_{\max}\}\)
throughout.
...
\(\Time_D(n)\leq C_L(|D_{\mathcal L}|+f_D(n)+1)^8\).  TM to term:
fuel \(c_DT(T+|u|)\). \\
...
Figure~\ref{fig:lambda-preservation} records the two exponent calculations of
its proof, whose maximum determines the universal rescaling \(A\).
\begin{figure}[H]
\centering
\input{figs/fig-lambda-preservation.tex}
\caption{The machine-to-term direction contributes exponent \(3+\lceil\log_2(72c_0)\rceil\), while the term-to-machine direction contributes \(8+\lceil\log_2(C_L3^8)\rceil\).  Choosing one universal \(A\) above both calculations preserves the bounded verifier class in either language.}
...
\[
 C_L(\lambda+n^\lambda+1)^8
 \leq C_L(3n^\lambda)^8
 \leq n^{(8+\lceil\log_2(C_L3^8)\rceil)\lambda}.
\]
...
\begin{proof}
By Lemma~\ref{lem:smn} with \(r=1\), let \(s(z,z)\) denote the description
obtained by hardwiring the string \(z\) into the first input of the program
\(z\) itself --- the same operation as in Part~I --- for a suitable
\((k+1)\)-input machine.  Define the partial computable function
\[
...
\begin{reminder}
The expression \(\mathcal F(\mathcal F,M,\lambda,\ldots)\) is the recursion theorem
...
operations before expressing the same construction as \(\Psi_{M,\lambda}\).
\begin{figure}[t]
\centering
\input{figs/fig-C-diagonal-five-steps.tex}
\caption{The machine \(\mathcal F\) first takes the amber early-halt branch or else hardwires its own description, computes \(S_\lambda\), calls \(\Compress\), and runs the returned decider (after \code{fig:halt_f}).  The dashed box isolates the cited compression guarantee from the green source operations that are actually performed.}
\label{fig:C-diagonal-five-steps}
\end{figure}

Let \(M:P\{\mathsf{MachineDesc}\}\) and
\(\lambda:P\{\mathsf{Nat}\}\) be closed literal terms, let \(S_\lambda\) be the sampler
description returned by \(\ComputeSampler(\lambda)\), and let \(n,x,y,a,b\) be the
five bound inputs.  The partial body corresponding to \code{fig:halt_f} is
\[
\begin{split}
 \Psi_{M,\lambda}=\lambda(n,x,y,a,b).\ \operatorname{If}(&
  \operatorname{Prim}(\mathsf{halts\_within},M,n),\ \mathsf{true},\\
 &\operatorname{Eval}(
   \operatorname{ans}(\operatorname{Eval}(
      \Quote(\Compress),\\[-2pt]
 &\quad (\operatorname{quoted\_pair}(\Quote(S_\lambda),
       \operatorname{Hole}(\mathsf{self\_code},
          \mathsf{Quoted}(\mathsf{Decider}))),\lambda),F_C)),\\[-2pt]
 &\quad(n,x,y,a,b),\operatorname{FuelBound}(n,\lambda))).
\end{split}
...
description.  Define
\[
 D_{M,\lambda}=\operatorname{YCode}(\Psi_{M,\lambda}),\qquad
 V_{M,\lambda}=(S_\lambda,\Quote(D_{M,\lambda})).
\tag{10.5}\label{eq:dml}
\]

Three points at which \eqref{eq:psi-ml} and the surrounding sections deviate
from the single source \code{docs/DESIGN.md} \S1.1 are declared here as
\grade{SOURCE\_REPAIR} against that file rather than left implicit.  First,
\S1.1's display carries one \(\Eval\) and neither an \(\mathsf{ans}\) selector
nor a fuel symbol for the compression call; \eqref{eq:psi-ml} uses two nested
\(\Eval\)s, \(\mathsf{ans}\), and \(F_C\), because the outer evaluation must
run the \emph{returned} decider and the inner one must be given a terminating
budget.  Second, \S1.1 writes the fixed-point equation with the \emph{same}
fuel on both sides, while Theorem~\ref{thm:ycode} charges \(f-c_Y\) with
\(c_Y=3\) for the self-code link.  Third, \S1.1 formerly fixed the hole sort
to \(\mathsf{Quoted}\{\mathsf{Decider}\}\), whereas
Section~\ref{sec:resource-lambda} admits \(\mathsf{Quoted}(A)\) for any result
sort \(A\); that repair has since been applied, and \code{DESIGN.md} \S1.1
now declares \(\mathsf{self\_code}:\mathsf{Quoted}\{A\}\), so on this point
the two agree.  The second has also been adopted: \code{DESIGN.md} \S1.1 now
carries \(\mathrm{eval}(\operatorname{Fix}(P),u;f)=\mathrm{eval}(\mathrm{specialize}(P,\{\mathsf{self\_code}\mapsto\Quote(\operatorname{Fix}P)\}),u;f-c_Y)\)
with \(c_Y=3\), and the TB4 rung pins it at every fuel on two machines
(\code{verdicts/tb4-r1.md}, \code{tb4-r2.md}).  At the byte level
\(\operatorname{Fix}\) serialises as one tag byte plus a 4-byte child count and
no sort annotation, so the link costs \(c_{\mathrm{fix}}=5\) bytes of description.
The first point was adjudicated the other way: the outer \(\Eval\) already
runs the returned decider, no \(\mathsf{ans}\) selector is needed because
the compressor returns the decider description (\code{gt-12} fig:halt\_f step~3
with lem:dhalt-values and lem:compress-independent-samplers), and no separate
\(F_C\) budget is needed because \(\operatorname{Compress}\) is total
polynomial time and is counted inside \(\mathrm{TIME}_D\) as lem:lambda
accounts it (\code{gt-12} L578--L596); \eqref{eq:psi-ml} keeps its two
\(\Eval\)s as a presentation and is not a repair.  The documents are now
in lockstep on all three points.
...
\caption{The literal bound \(\lambda\) produces \(S_\lambda\), while \(\operatorname{YCode}(\Psi_{M,\lambda})\) closes its sole self-code socket before quotation places \(D_{M,\lambda}\) beside that sampler.  The resulting \(V_{M,\lambda}\) is one finite normal-form verifier description, not an oracle for either component.}
...
\caption{The real term \(\Psi_{M,\lambda}\) first tests \(\mathsf{halts\_within}(M,n)\); on failure it evaluates quoted \(\Compress\) on \((\mathsf{quoted\_pair}(\Quote(S_\lambda),\operatorname{Hole}(\mathsf{self\_code})),\lambda)\), selects the answer decider, and evaluates it on \((n,x,y,a,b)\).  The lone amber hole is exactly the source position closed by \(D_{M,\lambda}=\operatorname{YCode}(\Psi_{M,\lambda})\).}
\label{fig:psi-ml}
\end{figure}
...
\((\overline M,\lambda)\) and inserts the resulting decider code into \(\Compress\);
\item the closed description \(D_{M,\lambda}\) in \eqref{eq:dml}.
\end{enumerate}
...
\(O(|M|+\log\lambda+1)\), and their running times differ by a polynomial in
\(|M|,\log\lambda,n\), the invoked fuel bounds, and the invoked machine times.
\end{theorem}
...
\caption{The published \(\mathcal F(\overline{\mathcal F},\overline M,\lambda,\ldots)\), Kleene's \(e_Q=s(p,p)\), and \(D_{M,\lambda}=\operatorname{YCode}(\Psi_{M,\lambda})\) are three finite descriptions of the same partial function (after \code{fig:halt_f}).  Every edge preserves the self-code substitution and keeps size \(O(|M|+\log\lambda+1)\).}
...
\(D(n,x,y,a,b)=\mathcal F(\overline{\mathcal F},\overline M,\lambda,n,x,y,a,b)\).
...
on failure, applies \(\Compress\) to \((S_\lambda,e)\) and runs the returned decider.
...
operational equation for \(D_{M,\lambda}\).  Translating it to a machine with
Theorem~\ref{thm:l-to-tm}, or translating \(\mathcal F\) with
Theorem~\ref{thm:tm-to-l}, preserves that behavior.

The displayed term contains one copy of \(M\), one binary literal \(\lambda\), and
fixed code for the remaining operations.  Theorem~\ref{thm:ycode} therefore
gives \(|D_{M,\lambda}|=O(|M|+\log\lambda+1)\).  The paper's hardwiring and the size
...
\caption{The green path closes \(\Psi_{M,\lambda}\) with \(\operatorname{YCode}\) and emits the finite bytes of \(\overline{V_{M,\lambda}}\), after which the arrow crosses a dashed boundary to low-degree soundness, rigidity, repetition, and gap preservation.  Self-reference supplies the program consumed by those theorems; it supplies none of their analytic conclusions.}
