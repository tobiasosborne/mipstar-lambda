\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[lblsm,anchor=north west,align=left,text width=12.8cm] at (0,-3.55)
    {\(y_w\) is the Tseitin variable for ``row \(0\) shows
     \((\lambda,v,\sqcup)\) at this window''; \(x_{1,2,s}\) says ``row \(1\),
  ...
  \node[checked, anchor=north west, text width=12.2cm, align=center, font=\small]
    ...
    {\(a_{i_1}^{o_1}\ \vee\ b_{i_2}^{o_2}\ \vee\ (u_3)^{0}_{\,y_w}\ \vee\
      ...
      {\scriptsize\sffamily for every \(i_1,i_2\) and every
       \(o_1,o_2\in\{0,1\}\)}};
  \draw[xform] (6.4,-4.20) -- (6.4,-4.65);
  \node[tag check, anchor=north east] at (12.7,-6.05) {ONE BIT PER BLOCK};
  \node[lblsm,anchor=north west,align=left,text width=12.8cm] at (0,-6.35)
